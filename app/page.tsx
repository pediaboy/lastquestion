...
import {
  ArrowRight, Zap, Shield, Globe, Cpu, BarChart3, Headphones,
  Rocket, Star, ChevronDown, Monitor, Award, CheckCircle2
} from 'lucide-react'
...
function StatCounter({ target, suffix }: { target: number; suffix: string }) {
  ...
  return <div ref={ref} className="text-4xl md:text-5xl font-black text-gradient mb-2">{count}{suffix}</div>
}
...
const techStack = ['Next.js', 'React', 'TypeScript', 'Tailwind CSS', 'Node.js', 'PostgreSQL', 'Vercel', 'Prisma', 'Framer Motion', 'GSAP']
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <div className="mt-16 grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
              {[{l:'Website Cepat',s:'Sub-2s loading'},{l:'Mobile First',s:'Responsif 100%'},{l:'SEO Ready',s:'Google-friendly'},{l:'Garansi Revisi',s:'Sampai puas'}].map((item) => (
                <div key={item.l} className="glass-card rounded-xl p-4 border-glow-animate">
                  <div className="text-white font-semibold text-sm">{item.l}</div>
                  <div className="text-gray-500 text-xs mt-1">{item.s}</div>
                </div>
              ))}
            </div>
          </motion.div>
        </div>
        <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="absolute bottom-8 left-1/2 -translate-x-1/2 text-gray-600">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {whyUs.map((item, i) => (
              <motion.div key={item.title} initial="hidden" whileInView="visible" viewport={{ once: true }}
                ...
                <div className="w-12 h-12 rounded-xl bg-white/5 flex items-center justify-center mb-4 group-hover:bg-white/10 transition-all">{item.icon}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {howItWorks.map((item, i) => (
              <motion.div key={item.step} initial="hidden" whileInView="visible" viewport={{ once: true }}
                ...
                <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-black text-lg mx-auto mb-4 shadow-lg shadow-indigo-500/30">{item.step}</div>
                <h3 className="text-white font-bold text-lg mb-2">{item.title}</h3>
                <p className="text-gray-400 text-sm leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {portfolioPrev.map((item, i) => (
              <motion.div key={item.name} initial="hidden" whileInView="visible" viewport={{ once: true }}
                variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, delay: i * 0.1 } } }}
      ...
      <section className="py-16 border-y border-white/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {techStack.map((tech) => (
              <div key={tech} className="glass-card px-5 py-2.5 rounded-full text-gray-300 text-sm hover:text-white hover:border-indigo-500/30 transition-all cursor-default">{tech}</div>
            ))}
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            {pricingPrev.map((item, i) => (
              <motion.div key={item.name} initial="hidden" whileInView="visible" viewport={{ once: true }}
                ...
                {item.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-bold px-4 py-1 rounded-full">PALING POPULER</div>
                )}
                <div className="text-gray-400 text-sm mb-1">{item.name}</div>
                <div className="text-3xl font-black text-white mb-1">Rp{item.price}</div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
            <div className="glass-card neon-border rounded-3xl p-12 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <h2 className="text-4xl md:text-5xl font-black mb-4 text-gradient">Siap Memulai?</h2>
                <p className="text-gray-400 text-lg mb-8 max-w-xl mx-auto">Konsultasikan kebutuhan website Anda sekarang. Gratis, tanpa komitmen, tanpa tekanan.</p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
